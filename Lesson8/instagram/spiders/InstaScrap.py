...
class InstascrapSpider(scrapy.Spider):
    ...
    def parse_followers(self, response: HtmlResponse, user_id, username):
        ...
        if j_data['big_list'] is True:
            ...
            self.parse_followers_data(j_data, username)
            ...
            url_followings = f'https://i.instagram.com/api/v1/friendships/{user_id}/followers/?count=12'
    ...
    def parse_followings(self, response: HtmlResponse, user_id, username):
        ...
        if j_data['big_list'] is True:
            next_max_id = j_data['next_max_id']
            self.parse_followings_data(j_data, username)
    ...
    def parse_followers_data(self, j_data, username):
        for follower in j_data['users']:
            ...
            item = InstagramItem(follower_name=follower_name, follower_id=follower_id, follower_photo_link=follower_photo_link, username=username)

            yield item


    def parse_followings_data(self, j_data, username):
        for following in j_data['users']:
            ...
            item = InstagramItem(following_name=following_name, following_id=following_id, following_photo_link=following_photo_link,
                                 username=username)
